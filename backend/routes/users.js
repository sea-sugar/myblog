...
const multer = require('multer'); //用于处理“multipart/form-data”类型数据格式的中间件，主要用于上传文件；该中间件在解析完请求体后，会向Request对象中添加一个body对象和一个file或files对象
...
const userSchema = new mongoose.Schema({
  ...
  users.use(express.json());
    // 加载和解析私钥
    const privateKeyData = fs.readFileSync('./rsakey/private.pem', 'utf8');
  ...
  // 处理忘记密码请求的路由接口
  users.post('/forgot',async(req,res) =>{  //req 和res 分别是什么？
  ...
  // 处理修改密码的接口
  users.post('/modifypassword', async (req, res) => {
  ...
  //处理修改用户名的接口
users.post('/modifyusername',async(req,res)=>{
  ...
  users.post('/uploadavatar', (req, res) => {
    const upload = multer({ dest: '../public/images' }); // 指定文件保存的目录
    ...
    upload.single('file')(req, res, err => {
      if (err) {
        ...
        const newFilePath = path.join(__dirname, '../public/images', fileName); // 保存的新路径
        ...
        fs.rename(filePath, newFilePath, err => {
          if (err) {
            ...
            // 处理保存文件成功的情况
            const imageUrl = `http://10.117.32.70:3000/images/${fileName}`; // 返回图片的URL
            res.json({ imageUrl });
